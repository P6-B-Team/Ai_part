...
# Committed data foundation — see app/rules/student_risk.py's module
# docstring for why this CSV (not the WST schema's own tables) is the
# permanent basis for this feature.
DATA_PATH = Path(__file__).resolve().parents[2] / "data" / "students_performance.csv"
MODEL_DIR = Path(__file__).resolve().parents[2] / "models"
MODEL_DIR.mkdir(exist_ok=True)

EARLY_FEATURES = ["Midterm_Score", "Assignments_Avg", "Quizzes_Avg", "Participation_Score",
                   "Study_Hours_per_Week", "Attendance (%)", "Stress_Level (1-10)", "Sleep_Hours_per_Night"]
FINAL_FEATURES = EARLY_FEATURES + ["Final_Score", "Projects_Score"]
...
def rule_prediction_early(row) -> int:
    result = evaluate_student_risk(StudentRiskFeatures(
        student_id=row["Student_ID"], midterm_score=row["Midterm_Score"],
        assignments_avg=row["Assignments_Avg"], quizzes_avg=row["Quizzes_Avg"],
...
def rule_prediction_final(row) -> int:
    result = evaluate_student_risk(StudentRiskFeatures(
        ...
        assignments_avg=row["Assignments_Avg"], quizzes_avg=row["Quizzes_Avg"],
        participation_score=row["Participation_Score"],
        final_score=row["Final_Score"], projects_score=row["Projects_Score"],
    ))
    return 1 if result.risk in ("High", "Medium") else 0
...
def run_variant(df, features, rule_fn, variant_name):
    print(f"\n{'=' * 70}\nVARIANT: {variant_name}  (features: {features})\n{'=' * 70}")

    train_val_df, test_df = train_test_split(
        df, test_size=config.TRAIN_TEST_SPLIT, random_state=42, stratify=df["at_risk"]
    )
    ...
    # Deterministic rule baseline, same held-out test set
    y_pred_rule = test_df.apply(rule_fn, axis=1).values
    ...
    print(f"Rule baseline: precision={rule_precision:.3f} recall={rule_recall:.3f} f1={rule_f1:.3f}")
    ...
    for key, model in CANDIDATES.items():
        ...
        cv_auc = roc_auc_score(y_train_val, oof_proba)

        gate_hit = best_threshold_for_gate(y_train_val, oof_proba)
        if gate_hit:
            threshold, selection_note = gate_hit[2], "clears precision+recall gate (CV)"
        else:
            _, _, threshold = best_f1_threshold(y_train_val, oof_proba)
            selection_note = "gate not clearable; using best-F1 threshold (CV)"
        ...
        print(f"  {key:20s} CV-AUC={cv_auc:.3f} threshold={threshold:.3f} "
              f"TEST precision={p_test:.3f} recall={r_test:.3f} f1={f1_test:.3f}  ({selection_note})")

        candidate_results[key] = {
            "model": final_model, "cv_auc": round(float(cv_auc), 3),
            "threshold": round(float(threshold), 3), "selection_note": selection_note,
            "test_precision": round(float(p_test), 3), "test_recall": round(float(r_test), 3),
            "test_f1": round(float(f1_test), 3),
        }

    # Pick the candidate with the best test F1 (test set used only for
    # this final reporting/selection step, never for threshold tuning)
    best_key = max(candidate_results, key=lambda k: candidate_results[k]["test_f1"])
    best = candidate_results[best_key]
    gate_passed = best["test_precision"] >= config.RISK_PRECISION_TARGET and best["test_recall"] >= config.RISK_MIN_RECALL_FOR_GATE
    ...
    metrics = {
        ...
        "dataset": {
            ...
            "total_rows": len(df), "train_val_rows": len(train_val_df), "test_rows": len(test_df),
            "target": "at_risk = Grade in ('D','F')", "positive_rate": round(float(df["at_risk"].mean()), 3),
        },
        "rule_baseline": {"precision": round(float(rule_precision), 3), "recall": round(float(rule_recall), 3), "f1": round(float(rule_f1), 3)},
        "all_candidates": {
            k: {kk: vv for kk, vv in v.items() if kk != "model"} for k, v in candidate_results.items()
        },
        "release_gate": {
            "target": f"Precision >= {config.RISK_PRECISION_TARGET:.0%} AND Recall >= {config.RISK_MIN_RECALL_FOR_GATE:.0%}",
            "gate_passed": bool(gate_passed),
            "selected_metrics": {"precision": best["test_precision"], "recall": best["test_recall"], "f1": best["test_f1"]},
        },
    }

    joblib.dump({"model": best["model"], "threshold": best["threshold"], "features": features, "variant": variant_name},
                MODEL_DIR / f"student_risk_{variant_name}_model.joblib")
    with open(MODEL_DIR / f"student_risk_{variant_name}_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2, default=str)

    print(f"\nSelected: {best_key} | gate_passed={gate_passed} | "
          f"precision={best['test_precision']} recall={best['test_recall']}")
    return metrics
...
def main():
    ...
    early_metrics = run_variant(df, EARLY_FEATURES, rule_prediction_early, "early")
    final_metrics = run_variant(df, FINAL_FEATURES, rule_prediction_final, "final")

    print(f"\n{'=' * 70}\nSUMMARY\n{'=' * 70}")
    print(f"early: precision={early_metrics['release_gate']['selected_metrics']['precision']} "
          f"recall={early_metrics['release_gate']['selected_metrics']['recall']} "
          f"gate_passed={early_metrics['release_gate']['gate_passed']}")
    print(f"final: precision={final_metrics['release_gate']['selected_metrics']['precision']} "
